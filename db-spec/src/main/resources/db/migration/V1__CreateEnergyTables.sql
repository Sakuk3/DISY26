...
CREATE TYPE energyEventTypes AS ENUM ('production', 'consumption');

CREATE TABLE energy_events
(
    id             BIGSERIAL PRIMARY KEY,
    association_id BIGINT           NOT NULL REFERENCES associations (id),
    event_type     energyEventTypes NOT NULL,
...
(
    ...
    community_produced_kwh NUMERIC(12, 3) NOT NULL DEFAULT 0,
    community_used_kwh     NUMERIC(12, 3) NOT NULL DEFAULT 0,
    grid_used_kwh          NUMERIC(12, 3) NOT NULL DEFAULT 0,
    created_at             TIMESTAMPTZ    NOT NULL DEFAULT now(),
...
(
    ...
    grid_dependency_pct  NUMERIC(5, 2),
    created_at           TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at           TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (association_id, hour_bucket)
);
